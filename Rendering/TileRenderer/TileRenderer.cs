...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.highway && (int)p.Value.PropValues == 8))
        {
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.water) && feature.Type != GeometryType.Point)
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.railway))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.landuse && ((int)p.Value.PropValues == 9)))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key== MapFeatureData.EnumKeysProp.landuse && ((int) p.Value.PropValues == 10)))
        {
            var coordinates = feature.Coordinates;
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == MapFeatureData.EnumKeysProp.landuse && ((int) p.Value.PropValues == 11)))
        {
            var coordinates = feature.Coordinates;
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                 ...
                 feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.landuse && ((int)p.Value.PropValues == 12)))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key== MapFeatureData.EnumKeysProp.leisure))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.EnumKeysProp.amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
